...
@interface QGEduClassViewController ()<UITableViewDataSource,UITableViewDelegate,UICollectionViewDelegateFlowLayout,UICollectionViewDataSource,UITextFieldDelegate>
...
#pragma mark 左边数据请求
- (void)addLeftRequest {
    [QGHttpManager eudactionDataSuccess:^(QGCategroyResultModel *result) {
        [self.categories removeAllObjects];
        self.categories = result.items;
        _categroyModel = _categories[0];

        NSLog(@"lllllllslssslllslsl %@ %@",_categroyModel.id,_categroyModel.name);
        [self loadCategroyInfos:_categroyModel.id];
...
-(void)loadCategroyInfos:(NSString *)reid
{
    ...
    QGGetGoodsListByCategoryIdHttpDownload *ghd = [[QGGetGoodsListByCategoryIdHttpDownload alloc]init];
    ghd.platform_id= [SAUserDefaults getValueWithKey:USERDEFAULTS_Platform_id];
    ghd.reid = reid;

    [QGHttpManager eudactionListDataWithParam:ghd Success:^(QGCategroyGoodsListResultModel *result) {

        [_rightSectionNames removeAllObjects];
        [_categorylists removeAllObjects];
        _rightSectionNames = [NSMutableArray new];
        _categorylists = [NSMutableArray new];

        for (QGCategroyGoodsListModel *model in result.items) {
            [self.rightSectionNames addObject:model.name];

            [_categorylists addObject:model.sublist];
...
{   NSArray *nn = _categorylists[section];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSArray *nn = _categorylists[indexPath.section];
    QGSublistModel *model = nn[indexPath.row];
    cell.model = model;
    return cell;
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    sear.CateID = model.id.integerValue;
    sear.searchOptionType= QGSearchOptionTypeCourse;
