...
//#if 1
    ...
    _minimumDate = [self dateWithYear:[self yearOfDate:now] month:[self monthOfDate:now] day:1];
    ...
    _placeholderType = FSCalendarPlaceholderTypeFillHeadTail;
    ...
    if (!self.floatingMode) {
        ...
        if (!_weekdays.count) {
            ...
            for (int i = 0; i < weekSymbols.count; i++) {
                ...
                NSString *str = weekSymbols[i];
                str = [str substringFromIndex:1];
                weekdayLabel.text = str;
...
- (void)invalidateWeekdaySymbols
{
    ...
    [_weekdays enumerateObjectsUsingBlock:^(UILabel *label, NSUInteger index, BOOL *stop) {
        ...
        index %= 7;
        NSString *str = useDefaultWeekdayCase ? weekdaySymbols[index] : [weekdaySymbols[index] uppercaseString];
...
- (void)reloadDataForCell:(FSCalendarCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *str = [self subtitleForDate:cell.date];
    cell.subtitle  = str;
...
//    cell.titleLabel.font = [UIFont boldSystemFontOfSize:14.0];
    if (cell.dateIsToday) {
        cell.titleLabel.font = [UIFont boldSystemFontOfSize:12.0];
        cell.title = @"今天";
    }else{
        cell.titleLabel.font = [UIFont boldSystemFontOfSize:14.0];
    }
    ...
    if (str.length < 1 && !cell.dateIsToday) {
        cell.canSelect = NO;
    }
...
- (NSString *)subtitleForDate:(NSDate *)date
{
    ...
    NSString *dateStr = [NSString stringWithFormat:@"%ld-%02ld-%02ld",[self yearOfDate:date],(long)[self monthOfDate:date],(long)[self dayOfDate:date]];
    NSValue *va = [_dataDic valueForKey:dateStr];
    if (va) {
        return [NSString stringWithFormat:@"%@场",va];
    }
//    NSLog(@"%@",_dataDic);
...
- (NSDate *)minimumDateForCalendar
{
    ...
    if (!minimumDate) {
        minimumDate = [self dateWithYear:[self yearOfDate:[NSDate date]] month:[self monthOfDate:[NSDate date]] day:1];
    }
    return minimumDate;
#endif
}
