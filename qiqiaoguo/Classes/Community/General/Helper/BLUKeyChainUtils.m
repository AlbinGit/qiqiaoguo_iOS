...
NSString *BLUKeyChainServiceName = @"BLUKeyChainServiceName";
NSString *BLUKeyChainUsername = @"BLUKeyChainUsername";

static NSString *BLUKeychainUtilsErrorDomain = @"BLUKeychainUtilsErrorDomain";

@implementation BLUKeyChainUtils

+ (NSString *) getPasswordForUsername: (NSString *) username andServiceName: (NSString *) serviceName error: (NSError **) error {
    
    if (!username || !serviceName) {
        if (error != nil) {
            *error = [NSError errorWithDomain: BLUKeychainUtilsErrorDomain code: -2000 userInfo: nil];
        }
        return nil;
    }
    ...
    if (status != noErr) {
        ...
        if (error != nil && status != errSecItemNotFound) {
            ...
            *error = [NSError errorWithDomain: BLUKeychainUtilsErrorDomain code: status userInfo: nil];
        }
        return nil;
    }
    ...
    if (status != noErr) {
        if (status == errSecItemNotFound) {
            ...
            if (error != nil) {
                *error = [NSError errorWithDomain: BLUKeychainUtilsErrorDomain code: -1999 userInfo: nil];
            }
        }
        else {
