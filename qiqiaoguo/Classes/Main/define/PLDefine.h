//
//  PLDefine.h
//  ToysOnline
//
//  Created by Albin on 14-8-19.
//  Copyright (c) 2014年 platomix. All rights reserved.
//


// 第三方
#import <Mantle/Mantle.h>
#import <AsyncDisplayKit/AsyncDisplayKit.h>

#import "IQKeyboardManager.h"
#import "QGTypeView.h"
//分类
#import "UIImage+MQimage.h"
#import "UIBarButtonItem+MQExtension.h"
#import "UIView+Extension.h"
#import "UIImage+MQimage.h"
#import "UIView+PLLayout.h"
#import "UIButton+MSMethod.h"
#import  "UIColor+BLUAddition.h"
#import "SAAlert.h"
//model
#import "QGCommon.h"



// control
#import "SAButton.h"
#import "SATextField.h"

#import "SATableView.h"


#import "SATextView.h"
#import "SAView.h"
#import "SAImageView.h"
#import "SAUtils.h"
#import "SASDateUtils.h"
#import "SAAppUtils.h"
#import "SAUserDefaults.h"
#import "SAProgressHud.h"
#import "SALabel.h"
#import "QGAddBtnView.h"
// define
//#import "PLUtilsDefine.h"
#import "SAAppDefine.h"

//请求
#import "AFNetworking.h"
#import "QGHttpDownloadConst.h"
#import "QGHttpManager+Home.h"
#import "SAUserDefaults.h"
#import "QGHttpManager.h"
#import "QGNotifiCationConst.h"
#import "QGHttpManager+Activity.h"
//MJExtension
#import "MJExtension.h"
//#import "Masonry.h"
//#import "UIImageView+WebCache.h"
//#import "MJRefresh.h"
#import "MBProgressHUD.h"

#import "QGHttpDownload+Education.h"
#import "QGHttpManager+QGOptimaiProduct.h"
//#import "QGHttpManager+User.h"



#define LOADING_480_200  [UIImage imageNamed:@"loading_480_200"]
#define LOADING_200  [UIImage imageNamed:@"cube_placeholder"]
#define LOADING_ACT [UIImage imageNamed:@"cube_placeholder"]
#define LOADING_HEAD [UIImage imageNamed:@"icon_message_head"]

#define UM_KEY @"534b9d3156240b3cdb0f4ad6123"
#define QQAppId @"100424468"
#define QQAppKey @"c7394704798a158208a74ab60104f0ba"
