...
- (void)initialization
{
    ...
    _titleLabelTextFont= [UIFont boldSystemFontOfSize:11];
                               ...
                               completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError){
                                   if (!connectionError) {
                                       ...
                                       dispatch_async(dispatch_get_main_queue(), ^{
                                           if (index == 0) {
                                               [self.mainView reloadData];//刷新
                                           }
                                       });
                                       [data saveDataCacheWithIdentifier:url.absoluteString];//缓存到本地
                                   } else { // 加载数据失败
                                       static int repeat = 0;
                                       dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                                           if (repeat > 10) return;//重复加载10次 10次加载不成功 不在加载
...
- (void)setupTimer
{
    ...
    _timer = timer;
    [[NSRunLoop mainRunLoop] addTimer:timer forMode:NSRunLoopCommonModes];//添加到runloop中
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.backgroundColor=PL_COLOR_255;
