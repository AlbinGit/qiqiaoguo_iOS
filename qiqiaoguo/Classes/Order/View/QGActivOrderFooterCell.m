...
@interface QGActivOrderFooterCell ()
...
@end
@implementation QGActivOrderFooterCell
...
//    
    ...
    _meassageLabel.Y = _goodsTotalLabel.maxY + 8;
...
//    
    self.cellSize = CGSizeMake(self.contentView.width, _creatTimeLabel.maxY + 8);
}
...
//    _postagePriceLabel.text = [NSString stringWithFormat:@"¥%@",order.deliveryAmount];
    ...
    if (order.orderMark.length > 0) {
        _meassageDetailLabel.text = order.orderMark;
    }
...
//            _deliveryTimeDetailLabel.text = @"已取消";
            break;
        }
        default:
