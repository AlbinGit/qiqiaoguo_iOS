...
#import "QGEducationOrderListViewController.h"
...
@interface QGEducationOrderListViewController () <UITableViewDelegate,UITableViewDataSource,QGSelectOrderCancelReasonDelegate>
...
@implementation QGEducationOrderListViewController
- (instancetype)init
{
    self = [super init];
    if (self) {
...
- (void)viewDidLoad{
    ...
    [BLURefreshHeader headerWithRefreshingBlock:^{
        @strongify(self);
        self.page = 1;
        [QGHttpManager getEduOrderListWithPage:self.page orderStatus:self.orderStatus Success:^(NSURLSessionDataTask *task, id responseObject) {
...
- (void)goToStore:(UIButton *)button{
    ...
    vc.org_id =[NSString stringWithFormat:@"%ld",(long)order.Sid];
    [self.navigationController pushViewController:vc animated:YES];
