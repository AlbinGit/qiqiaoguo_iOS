...
@interface QGActivOrderDetailViewController () <UITableViewDataSource,UITableViewDelegate,QGSelectOrderCancelReasonDelegate>
...
@implementation QGActivOrderDetailViewController
...
- (void)viewDidLoad{
    ...
    [_tableView registerClass:[QGActivOrderHeaderCell class] forCellReuseIdentifier:NSStringFromClass([QGActivOrderHeaderCell class])];
    ...
    [_tableView registerClass:[QGActivOrderDetailCell class] forCellReuseIdentifier:NSStringFromClass([QGActivOrderDetailCell class])];
    ...
    [BLURefreshHeader headerWithRefreshingBlock:^{
        @strongify(self);
        [QGHttpManager getActivOrderDetailWithOrderID:self.orderID Success:^(NSURLSessionDataTask *task, id responseObject) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    if (indexPath.row == 0) {
        QGActivOrderHeaderCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([QGActivOrderHeaderCell class]) forIndexPath:indexPath];
        ...
        QGActivOrderDetailCell *cell =[tableView dequeueReusableCellWithIdentifier:NSStringFromClass([QGActivOrderDetailCell class]) forIndexPath:indexPath];
...
heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == 0) {
        CGSize size = [_tableView sizeForCellWithCellClass:[QGActivOrderHeaderCell class] cacheByIndexPath:indexPath width:self.view.width configuration:^(QGCell *cell) {
            QGActivOrderHeaderCell *headcell = (QGActivOrderHeaderCell *)cell;
    ...
    CGSize size = [_tableView sizeForCellWithCellClass:[QGActivOrderDetailCell class] cacheByIndexPath:indexPath width:self.view.width configuration:^(QGCell *cell) {
        QGActivOrderDetailCell *footcell = (QGActivOrderDetailCell *)cell;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 1) {
        QGActivOrderModel *order = self.order;
        QGActivityDetailViewController *vc = [QGActivityDetailViewController new];
        vc.activity_id = [@(order.activID) stringValue];
...
- (void)submitAction:(UIButton *)button {
    ...
    if ([order isKindOfClass:[QGMallOrderModel class]]) {
        ...
        vc.activity_id =  [@(Acitvorder.activID) stringValue];
        [self.navigationController pushViewController:vc animated:YES];
    }
}
