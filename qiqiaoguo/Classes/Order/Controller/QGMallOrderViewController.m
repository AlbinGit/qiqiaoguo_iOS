...
@interface QGMallOrderViewController () <VTMagicViewDataSource,VTMagicViewDelegate>
...
@implementation QGMallOrderViewController
...
- (NSArray<NSString *> *)menuTitlesForMagicView:(VTMagicView *)magicView
{
    ...
    [titleList addObjectsFromArray:@[@"全部订单",@"待付款",@"待发货",@"待收货"]];
    _menuList = titleList;
    return _menuList;
}
...
- (UIViewController *)magicView:(VTMagicView *)magicView viewControllerAtPage:(NSUInteger)pageIndex
{
    if (pageIndex == 0) {
        ...
        QGMallOrderListViewController *viewController = [magicView dequeueReusablePageWithIdentifier:gridId];
        if (!viewController) {
            viewController = [[QGMallOrderListViewController alloc] init];
    ...
    if (pageIndex == 2) {
        static NSString *gridId = @"daifahuo";
        QGMallOrderListViewController *viewController = [magicView dequeueReusablePageWithIdentifier:gridId];
        if (!viewController) {
            viewController = [[QGMallOrderListViewController alloc] init];
            viewController.orderStatus = QGMallOrderStatusSend;
        }
        return viewController;
    }
    if (pageIndex == 3) {
        static NSString *gridId = @"daishouhuo";
        QGMallOrderListViewController *viewController = [magicView dequeueReusablePageWithIdentifier:gridId];
        if (!viewController) {
            viewController = [[QGMallOrderListViewController alloc] init];
            viewController.orderStatus = QGMallOrderStatusDidSend;
        }
        return viewController;
    }
    return nil;
